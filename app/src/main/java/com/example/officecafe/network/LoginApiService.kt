...
object KotlinClient
{
    ...
    private val newRetrofit =Retrofit.Builder().baseUrl(BASE_URL).addConverterFactory(GsonConverterFactory.create(
        gson)).build()

    val api: LoginApiService = newRetrofit.create(LoginApiService::class.java)
...
interface LoginApiService {
    ...
    @POST("getbalance")
    suspend fun requestGetBalance(@Header("Authorization") token: String, @Body requestModel: GetBalanceRequest): Response<GetBalanceResponse>

    @POST("creditNoteApi")
    suspend fun requestCreditNote(@Header("Authorization") token: String, @Body requestModel: CreditCashRequest): Response<CreditNoteResponse>
}
